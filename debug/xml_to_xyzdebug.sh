...
#    echo  ${energies_array[${i}*2+1]}
    # Set all energies to absolute value
    energyabs=`echo "sqrt("${energies_array[${i}*2+1]}"^2)" |  bc -l `
    #energieperatom=`echo ${energies_array[${i}*2+1]}"/2" |  bc -l `
    echo $energyabs
    energyperatom=`echo $energyabs"/2" |  bc -l `

    echo $natom >> $outfile
    echo "Lattice=\"$lattice\" Properties=species:S:1:pos:R:3:forces:R:3:energies:R:1 energy=$energyabs pbc=\"T T T\"" >> $outfile
    ...
    do
        ...
        ${for_array[${i}*${natom}*3+${j}*3+2]} $energyperatom >> $outfile
    done
